...
// export async function checkRedis(redisClient?: Redis, msTimeout = 300): Promise<CheckResult> {
//   if (!redisClient) return { ok: true, details: "redis-not-configured" };
//   try {
//     const pong = await withTimeout(redisClient.ping(), msTimeout);
...
export function ensureHealthy(opts: {
  ...
  return asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    // if (checks.redis && redisClient) {
    //   const r = await checkRedis(redisClient);
    //   results.redis = r;
    //   if (!r.ok) allOk = false;
    // } else if (checks.redis) {
    //   results.redis = { ok: true, details: "redis-not-configured" };
    // }
